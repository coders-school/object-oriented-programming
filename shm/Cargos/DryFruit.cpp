...
bool DryFruit::operator==(const Cargo& other) const {
    if (const auto* otherDryFruit = dynamic_cast<const DryFruit*>(&other)) {
        return name_ == otherDryFruit->getName() and this->getPerishable() == otherDryFruit->getPerishable();
    }
    return false;
}
...
std::unique_ptr<Cargo> DryFruit::createAmountOfEqual(size_t amount) {
    return std::make_unique<DryFruit>(name_, amount, basePrice_, getFreshTime(), getMaxFreshTime());
}
