...
Store::Response Store::buy(Cargo* cargo, size_t amount, Player* player) {
    ...
    if (this->getCargo(cargo->getName()) != nullptr) {
        size_t totalPrice = this->getCargo(cargo->getName())->getBasePrice() * amount;
        ...
        if (player->getAvailableSpace() < this->getCargo(cargo->getName())->getAmount())
...
Store::Response Store::sell(Cargo* cargo, size_t amount, Player* player) {
    if (this->getCargo(cargo->getName()) != nullptr) {
        size_t totalPrice = player->getCargo(cargo->getName())->getBasePrice() * amount;
        ...
        if (this->getAvaiableSpace() < player->getCargo(cargo->getName())->getAmount())
            return Response::lack_of_space;
    ...
    if (mode == BUY) {
        ...
        if (this->getCargo(cargo->getName())->getAmount() == amount) {
            auto it = std::find_if(cargoVec.begin(), cargoVec.end(), [=](Cargo* el) {
                return el->getName() == cargo->getName();
            });  // przerobic getCargo na prawidlowe
            cargoVec.erase(it);
        ...
        if (this->getCargo(cargo->getName()) != nullptr) {
            ...
            std::cout << "You cant sell this type of good i stroe." << std::endl;
        }
    }
}
